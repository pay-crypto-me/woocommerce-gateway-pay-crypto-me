...
- i18n: strings do cliente e **rótulos/descrições de settings** via `__()`/`esc_html__()` com text domain `paycrypto-me-for-woocommerce`. **Erros, warnings, logs, feedback de botão de diagnóstico e notas de pedido do admin ficam em inglês literal**, sem `__()` — a regra completa e o porquê estão em [TRANSLATION.md](./TRANSLATION.md) → "O que entra (e o que NÃO entra) no catálogo". Não envolva uma mensagem de erro nova em `__()` só porque ela aparece na tela.
...
- Se o gateway novo tocar as libs `bitwasp/*`, **passe pelo `BitcoinAddressService`** em vez de chamá-las direto: elas emitem `E_DEPRECATED` no meio da requisição (o que já quebrou o redirect pós-save das settings On-Chain), e a máscara escopada vive lá — ver `suppress_vendor_deprecations()` e [CRYPTO-DEPRECATION-CONTINGENCY.md](./CRYPTO-DEPRECATION-CONTINGENCY.md). Nunca capture `\Throwable` na validação: extensão ausente é `\Error` e tem de propagar.
- Dois guards da suíte alcançam arquivo novo automaticamente, então não há passo a fazer — só não os contorne: `VendorReplaceGuardTest` varre **todo** PHP enviado (`includes/`, `exceptions/`, `templates/`, entrypoint, `uninstall.php`) proibindo `lastguest\Murmur`/`murmur3`/`BloomFilter` (pacote removido via `replace` — ver [LEAN-VENDOR-TREE.md](./LEAN-VENDOR-TREE.md)), e `PhpFloorConsistencyTest` fixa o piso de PHP nos quatro lugares que o declaram, caso o gateway novo precise subir esse piso.
