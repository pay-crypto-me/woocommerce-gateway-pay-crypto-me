...
- **[PLAN — NOT STARTED]** [docs/PLAN-SCHEMA-INSTALL-HARDENING.md](docs/PLAN-SCHEMA-INSTALL-HARDENING.md) — closes the six findings of the independent adversarial review of `fix/schema-upgrade-and-static-records` (2026-08-28). The two that matter: (1) the post-lock `is_current()` recheck also gates the **activation hook**, so a site with the version option recorded but a table missing is never repaired — measured against real MySQL, and every published install has that option set; (2) `$wpdb->last_error` only survives `dbDelta`'s **last** statement, so the first `DB_VERSION` bump that adds a column *and* an index can record success over a broken schema. Also: `insert_address()` conflating "row already exists" with "insert failed" (a double-submitted order is told its payment failed), plus the doc corrections — `upgrader_process_complete` does **not** cover an admin-UI update of this plugin, only WP-CLI/cron. Carries its own measured evidence, traps, 26-test plan and a 20-row acceptance table; written to be executed by a fresh agent with only that file and the codebase. **Nothing in it is in the code yet.**
...
- **`install()` is serialized** on the `paycrypto_me_db_install` advisory lock (same
  ...
  notice must not fire for something that resolves itself.
...
| `DbInstaller` | `services/class-db-installer.php` | Activation/upgrade of the 4 custom tables + the failed-install admin notice; owns `DB_VERSION`. `install()` is serialized on the `paycrypto_me_db_install` advisory lock; `maybe_upgrade()` is forward-only (`version_compare`) and hooked on `admin_init`/`upgrader_process_complete`, never on the front-end path; `is_current()` is what other code asks before depending on a newer schema |
